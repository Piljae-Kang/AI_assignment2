...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            possiple_position.append((x, y))

    variableDomainsDict[PAC] = possiple_position
    variableDomainsDict[GHOST0] = possiple_position
    variableDomainsDict[GHOST1] = possiple_position

    max_distance = -1
    for xy1 in possiple_position:
        for xy2 in possiple_position:
            distance = manhattanDistance(xy1, xy2)
            if max_distance < distance:
                max_distance = distance

    for i in range(0, max_distance + MAX_NOISE + 1):
        distance_history.append(i)
    
    variableDomainsDict[OBS0] = distance_history
    variableDomainsDict[OBS1] = distance_history
